"""
Mask R-CNN
Train on the toy pothole dataset and implement color splash effect.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 pothole.py train --dataset=/path/to/pothole/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 pothole.py train --dataset=/path/to/pothole/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 pothole.py train --dataset=/path/to/pothole/dataset --weights=imagenet

    # Apply color splash to an image
    python3 pothole.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 pothole.py splash --weights=last --video=<URL or path to file>
"""
...
class PotholeDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        # Convert to a bitmap mask of shape
        # [height, width, instance_count]
        info = self.image_info[image_id]
        mask = np.zeros([info["height"], info["width"], len(info["start_xv"])],
                        dtype=np.uint8)
        for i in range(len(info["start_xv"])):
            sx = info["start_xv"][i]
            sy = info["start_yv"][i]
            ex = info["extent_xv"][i]
            ey = info["extent_yv"][i]
            rr, cc = skimage.draw.rectangle(start = (sy,sx), extent=(ey,ex)) # NOTE the flip here - why?
            mask[rr, cc, i] = 1

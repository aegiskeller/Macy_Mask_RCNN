...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 airbus.py train --dataset=/path/to/airbus/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 airbus.py train --dataset=/path/to/airbus/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 airbus.py train --dataset=/path/to/airbus/dataset --weights=imagenet

    # Apply color splash to an image
    python3 airbus.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 airbus.py splash --weights=last --video=<URL or path to file>
"""
...
class AirbusDataset(utils.Dataset):
    ...
    def load_airbus(self, dataset_dir, subset):
        ...
        # but seriously leave this as the same as the source
        # or it will fail
        self.add_class("airbus", 1, "airbus")
        height=768
        width=768
        ...
        if (subset == "val"):
            subset_datadir = os.path.join(dataset_dir, "train_v2") # the test_v2 is separate
        else:
            subset_datadir = os.path.join(dataset_dir, "train_v2")
        ...
        if count == 0:
            ...
            for i, a in enumerate(annotations):
                ...
                for lo, hi in zip(starts, ends):
                    img[lo:hi] = 1
                mask[:, :, i] =  img.reshape(shape).T  # Needed to align to RLE direction
...
def train(model):
    ...
    sys.exit()
    dataset_train.load_airbus(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = AirbusDataset()
    dataset_val.load_airbus(args.dataset, "val")
    dataset_val.prepare()
